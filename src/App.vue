...
let renderer, camera, scene, gui, stats, controls, selectionShape, cloud, helper;
...
function init() {
  ...
  function loadPointCloud() {
    ...
    // load Depthhelper
    cloud.geometry.boundsTree = new MeshBVH(cloud.geometry);
    cloud.castShadow = true;
    cloud.receiveShadow = true;
    group.add(cloud)
    helper = new MeshBVHVisualizer(cloud, 10)
    group.add(helper)
  }
  ...
  highlightMesh.geometry = cloud.geometry.clone();
  ...
  // gui
  gui = new GUI();
  const selectionFolder = gui.addFolder('点云选取套索工具');
  ...
  const displayFolder = gui.addFolder('性能优化选项');
...
function render() {
  ...
  cloud.material.wireframe = params.wireframe;
...
function updateSelection() {
      ...
      if (params.selectionMode === 'centroid' || params.selectionMode === 'centroid-visible') {
        ...
        if (
          ...
          if (params.selectionMode === 'centroid-visible') {
            ...
            const res = cloud.geometry.boundsTree.raycastFirst(tempRay, THREE.DoubleSide);
            if (res) {
  ...
  const indexAttr = cloud.geometry.index;
